...
string parsepar( vector<string> src )
{
 ...
 return output;
  ...
  bool AgendaUI::executeOperation(const std::string &t_operation)
  {
    ...
    {
    	cout << "Have you decided to delete this user? y / n" << endl;
    	char c;
    	cin >> c;
    	if( c == 'y' )
    	{
    ...
    {
    	cout << "If you really want to delete all the meetings? y / n " << endl;
    	char c;
    	cin >> c;
    	if( c == 'y' )
    	{
  ...
  void AgendaUI::userRegister(void)
  {
	...
	{
		cout << "[register] Your email address is invalid!" << endl;
		return;
	}*/
	if( phone.size() != 11 )
	{
  ...
  {
  	cout << "Have you decided to delete all meeting? y / n ";
  	char c;
  	cin >> c;
  	if( c == 'n' )
  	{
